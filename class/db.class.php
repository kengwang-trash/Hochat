...
class DB
{
     ...
     */
    public function __construct($host, $user, $password, $db, $prefix,$port=3306)
    {
        $this->config = array(
            'host' => $host,
            'user' => $user,
        ...
        try {
            ...
            $this->error = array('error' => true, 'msg' => $this->SQL->connect_error, 'en' => $this->SQL->connect_errno);
            ErrorHandle::Error(500, "数据库连接错误", 'MySQL Connect', '-50', array('dberr' => $this->SQL->connect_error));
            echo '数据库无法连接';
            exit(502);
        }
     ...
     */
    public function SelectData($table, $where = array(), $place = array(), $other = "")
    {
        if (count($where)) {
            $w = ' WHERE ';
        ...
        $sqll = "SELECT" . $p . ' FROM `' . $this->prefix . $table . '` ' . $w . $temp . $other . ';';
        ...
        if (count($where)) {
            if (array_keys($where) === range(0, count($where) - 1)) {
                ...
                foreach ($where as $item) {
                    ...
                    if (!$first) $temp .= ' and `' . $item['p'] . '` ' . $item['c'] . ' ' . $this->check_input($item['v']);
                    else $temp .= ' `' . $item['p'] . '` ' . $item['c'] . ' ' . $this->check_input($item['v']);
                    $first = false;
                }
            } else {
                ...
                foreach ($where as $k => $v) {
                    if (!$first) $temp .= ' and `' . $k . '` = ' . $this->check_input($v);
                    else $temp .= ' `' . $k . '` = ' . $this->check_input($v);
                    $first = false;
                }
            }
        }
     ...
     * @return array
     */
    public function SelectDataArray($table, $where = array(), $place = array(), $other = "")
    {
        $r = $this->SelectData($table, $where, $place, $other);
        if ($r) return $this->FetchResult($r, MYSQLI_BOTH);
        else return array();
    }
    ...
    {
        return array('error' => true, 'msg' => $this->SQL->error, 'en' => $this->SQL->errno);
    }
